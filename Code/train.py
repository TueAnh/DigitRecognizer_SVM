...
train = pd.read_csv('F:/Artificial Intelligence/SVM/Data/train_small_1300.csv')
X_train = train.iloc[:,1:].values
y = train.iloc[:,0].values
sns.countplot(y)
plt.show()
y_list = [np.where(y == i, 1.0, -1.0) for i in range(10)]

classifiers = []
#Start train
print(datetime.datetime.now())
for y_i in y_list:
    smo = SMO_SVM.SmoAlgorithm(X_train,y_i,C=10000, tol=0.001,kernel=linear_kernel,use_linear_optim = True)
    smo.main_routine()
    w = compute_w(smo.alphas, X_train, y_i)
    classifiers.append(np.append(w,[-smo.b]))
#End train
print("Complete Traning! {0}".format(datetime.datetime.now()))
#output model file
